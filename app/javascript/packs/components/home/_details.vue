 ...
 <v-layout row wrap>
  <v-flex md4 mb-3 mt-2 offset-md1 xs10 offset-xs1>
   ...
   <v-layout row wrap mt-3>
        ...
        <v-icon v-for="r in (5 - watchable.attributes.reviews_count)"
                :key='watchable.attributes.reviews_count + r'
                color="white">favorite</v-icon>
        </v-flex>
    </v-layout>
    <v-layout row wrap mt-4>
     ...
     <v-flex sm3 md4>
      <v-btn color="black" class="watch-btn white--text" large>+ MinhaLista</v-btn>
     </v-flex>
    </v-layout>
